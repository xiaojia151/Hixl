...
PagedAttention场景下，分配多个blocks的Cache，Cache分配成功后，可通过[deallocate\_blocks\_cache](deallocate_blocks_cache.md)释放内存。
...
执行时间超过[sync\_kv\_timeout](sync_kv_timeout.md)配置会抛出LLMException异常。
