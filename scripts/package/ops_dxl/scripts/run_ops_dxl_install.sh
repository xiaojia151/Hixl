#!/bin/bash
# Perform install for ops_dxl package
...
##########################################################################
log "INFO" "step into run_ops_dxl_install.sh ......"
...
new_install() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to install ops_dxl files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="ops_dxl" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall \
