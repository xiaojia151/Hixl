...
def __construct_header(
    ...
    head_hash = (
        zero_bytes_32  # fill in zero first, it will be calculated and filled later
    )
...
def __write_header(
    out, header, suffix=False, head_type=0, code_len=0, before_header=False
):
    header_base = 0x1000 if head_type else 0
...
def __write_header_hash(
    out, suffix=False, head_type=0, sm=False, code_len=0, before_header=False
):
    header_base = 0x1000 if head_type else 0
...
def __add_magic_number_and_file_size(
    ...
    file_size = 0 if before_header and (out.tell() >= 0x100000000) else out.tell()
    s = struct.Struct("QI")
    if cms_flag:
...
def write_extern(args, out, list):
    ...
    __add_magic_number_and_file_size(
        args, out, args.addcms, False, code_len, before_header
    )
    return
