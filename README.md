...
- [2025/10] ops-dxl项目首次上线。

## 🚀概述
Ascend dxl（Direct Xfer Library）是昇腾单边通信库，提供高性能、零拷贝的点对点数据传输能力，并通过简易API开放给用户。

<img src="docs/figures/arch.png" alt="架构图">
...
为了应对该挑战，Ascend dxl应运而生，在提供高带宽、低时延的点对点通信的同时，也为上层推理框架提供支持。Ascend dxl通过简易的API并利用昇腾集群多样化通信链路（RoCE/HCCS/UB），可实现跨实例和集群的高效KV Cache传输，支持与主流LLM推理框架vLLM等的集成，并可用于构筑分布式数据管理系统。
